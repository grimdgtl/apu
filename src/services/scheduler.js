...
export function startScheduler() {
  ...
  if (featureEnabled.birthdays) {
    cron.schedule(config.cron.birthdayMorning, () => jobs.pokreni('rodjendani najava', birthdayMorning), options);
    cron.schedule(config.cron.birthdayEvening, () => jobs.pokreni('rodjendani podsetnik', birthdayEvening), options);
    logger.info(
...
async function birthdayMorning() {
  ...
  try {
    ...
    logger.error('Greška pri proveri rođendana:', err.message);
    throw err; // propusti dalje da retry i evidencija poslova vide pad
  }
}
...
async function birthdayEvening() {
  ...
  try {
    ...
    logger.error('Greška pri večernjem podsetniku za rođendane:', err.message);
    throw err;
  }
}
